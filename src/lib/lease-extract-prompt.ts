...
export function buildLeaseTemplateReviewSystemPrompt() {
  return (
    ...
    "The document may contain {{double_brace}} placeholders (e.g. {{rent_amount}}, {{late_fee_amount}}, {{Sign;type=signature;role=Tenant 1}}). These are template tokens that are filled with real data when a lease is generated — treat them as valid provided values. Never flag a clause, missing concept, or inconsistency because a value is a placeholder or 'undefined'. " +
    "Findings must be actionable gaps, not commentary. Before reporting a missingConcept, check the ENTIRE document including addenda near the end — report it only when no clause anywhere covers that subject. Report a stateLawNote only when the current lease text fails to satisfy that requirement; do not note laws the lease already complies with. If the lease already covers an area adequately, leave it out — empty arrays are the correct output for a well-drafted lease; never pad the lists. " +
